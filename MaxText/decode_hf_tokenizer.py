...
def hf_encode(tokenizer: PreTrainedTokenizer, text, **kwargs):
  ...
  tokens, true_length = pad_tokens(
    ...
    is_bos=is_bos and tokenizer.bos_token_id is not None,
...
def main(config):
  ...
  tokens, true_length = hf_encode(tokenizer, text, is_bos=True, prefill_lengths=[config.max_prefill_predict_length])
