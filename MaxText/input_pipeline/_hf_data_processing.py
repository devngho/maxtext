...
def preprocessing_pipeline(
  ...
  dataloader = grain.DataLoader(
      ...
      read_options=grain.ReadOptions(num_threads=1 if random_access else num_threads, prefetch_buffer_size=512),
  )
...
def make_hf_iterator(
  ...
  max_logging.log(f'Training rows: {len(train_ds)} Eval rows: {len(eval_ds) if eval_iter is not None else "eval not enabled"}')
  max_logging.log('It doesn\'t real data count will be used as maxtext uses sequence packing.')
